...
1. **Routing** — when device _X_ wants to send a chat message to peer _B_, how does it
   know _which_ accounts to address the envelope to?
2. **Sync (MDS)** — when device _X_ establishes/changes a chat, how does its **sibling**
   device _X'_ (same user, paired) learn about it, in both directions?
...
| **identity chat key** (`identityChatPrivateKey` / `identityChatPublicKey`, X25519) | user   | Outer ECDH for the _identity channel_ (chat-requests, accept signals, roster fan-out). Derives the push topic. | one per user; **private half shared to every sibling via the V2 handshake** |
...
| **device encryption key** (`encryptionPublicKey`, X25519, 32 bytes)                | device | Per-device ECDH wrap of the one-shot message key inside a `MultiDeviceRequest`.                                | one per physical device                                                     |

Two **rosters** built from those keys:

- **OWN / sibling roster** — _my_ other paired devices `{statementAccountId, encryptionPublicKey}`.
  Source of truth differs per platform (SSO sessions on mobile; `device-sync` table on Desktop).
  Used to spawn sync channels and to fan out _my_ devices to a peer.
- **PEER roster** — _the other party's_ devices `{statementAccountId, encryptionPublicKey}`.
...
_its own_ `statementAccountId` and unwraps.
...
use the **identity-level** coder: ECDH against the peer's _identity_ chat key, no per-device
...
> _unknown_ sender statement account ⇒ `deviceEntryNotFound` ⇒ dropped. This is the
...
2. **Peer-device roster is replicated as `deviceAdded`/`deviceChatAccepted` _chat-message rows_,
...
Per-peer wall-clock checkpoint captured at the _start_ of the round, advanced only on Ack
...
A contact only syncs once its request is _established_ — otherwise a pending outgoing request
...
- **Chat-request _state_ itself is never a SyncEntity on any platform.**

---

## 3. Case matrix — Desktop sibling ⇄ Android sibling, chatting with peer B

Scenario: user _A_ is paired across **Desktop (A_d)** + **Android (A_a)**. They chat with peer _B_
(iOS or Android). "Hub" = the device that performed the action; "sibling" = the device that must
catch up via MDS. Peer _B_ is assumed on-chain registered unless noted.
...
3. fan out A_d's _sibling_ devices to B on the device channel (`:557-608`).
...
- `deviceChatAccepted(B,Incoming)` → saved; `ChatRequestAcceptProcessor` finds no pending _outgoing_ request on the sibling, no-op. Harmless (roster already set by `deviceAdded`).

**Verdict: works, _iff_ the chain resolve for B succeeds on A_a.**
...
- `ChatsAdded(B)` → `resolveConsumerInfo(B)` → `Contact(B){devices:[]}` + room (`:115-136`). ✅ _iff resolve succeeds._
...
`deviceAdded` _incoming_, bump `lastUpdate`, drop legacy `contactAdded` from the wire,
...
Desktop embeds the peer roster _inside_ the `Contact` entity (`Contact.devices[]`), so its
...
Consequence: whenever `ChatsAdded` resolution fails or arrives in a _different_ sync round than
...
transient chain failure loses the _entire_ chat on the sibling with no recovery path. Android
...
No own-roster producer (`collector.ts` has no `Devices` branch). All platforms ignore _inbound_
...
1. **`deviceAdded` / `deviceChatAccepted` for an unknown contact should not be dropped.**
   ...
   `ChatsAdded` (`ctx.resolveConsumerInfo(peerSs58)` → upsert `Contact` + room), _then_ apply the
