...
Feature: Link Navigation
  ...
  @allure.id:14759
  Scenario: TC-4.5.1 Client-side pushState on a visible tab updates the host route
    When the link-tests product dispatches a pushState to "/push-target"
    Then the host route pathname ends with "/push-target"

  # Extra coverage beyond plan 900 — anchor-click variant of TC-4.5.1; no distinct TestOps case.
  ...
  @allure.id:14760
  Scenario: TC-4.5.2 Backgrounded tab's pushState does not hijack the host route
  ...
  # Extra coverage beyond plan 900 — backgrounded-state preservation; no distinct TestOps case.
  ...
  @allure.id:14761
  Scenario: TC-4.5.3 Cross-product polkadot:// link does not replace the source tab's content
  ...
  @allure.id:14762
  Scenario: TC-4.5.4 Re-opening a backgrounded product's root via the address bar does not freeze the browser
