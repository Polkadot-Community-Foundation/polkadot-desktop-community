...
Given('a wedging product is loaded in a webview tab', ({ electronApp }) => {
...
When("the product's main thread wedges with an infinite loop", ({ electronApp }) => {
...
Then('the unresponsive overlay appears within {int} seconds', ({ electronApp }, _timeoutSeconds: number) => {
...
Then('clicking the reload button clears the overlay', ({ electronApp }) => {
