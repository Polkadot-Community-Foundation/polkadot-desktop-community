...
import { type contactRepository as defaultContactRepository } from '../identity/repository';
import { DeviceRosterEvent } from '../identity/schemas';
import { type Contact } from '../identity/types';

import { rosterUseCase } from './roster';
...
describe('startRosterSubscriber', () => {
  ...
  it('returns a no-op unsubscribe when there are no contacts', () => {
    ...
    const stop = rosterUseCase.startRosterSubscriber({
  ...
  it('subscribes with matchAny across roster topics for every known contact', () => {
    ...
    const adapter = makeStatementStoreCapturing(cap);
    rosterUseCase.startRosterSubscriber({ statementStore: adapter, contacts, contactRepository: inMemoryContactRepo() });
  ...
  it('applies a DeviceAdded event signed by the matching contact', async () => {
    ...
    const adapter = makeStatementStoreCapturing(cap);
    rosterUseCase.startRosterSubscriber({ statementStore: adapter, contacts: [contact], contactRepository: repo });
  ...
  it('processes events from multiple contacts in one stream (matchAny dispatches by signer)', async () => {
    ...
    const adapter = makeStatementStoreCapturing(cap);
    rosterUseCase.startRosterSubscriber({ statementStore: adapter, contacts: [a, b], contactRepository: repo });
