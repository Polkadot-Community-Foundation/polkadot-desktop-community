...
import { DeviceAdded, DeviceRemoved, DeviceRosterEvent } from './schemas';

const accountId = (fill: number) => new Uint8Array(32).fill(fill);
const publicKey = (fill: number) => new Uint8Array(32).fill(fill);
...
describe('DeviceAdded', () => {
  ...
  it('encodes with the expected fixed length (32 + 32 = 64 bytes)', () => {
    const e = { statementAccountId: accountId(0xa1), encryptionPublicKey: publicKey(0x04) };
    expect(DeviceAdded.enc(e).length).toBe(64);
  });
});
