...
// papi runtime-metadata values may expose their raw bytes via an `asBytes()`
// method — narrow by shape instead of casting.
const hasAsBytes = (value: unknown): value is { asBytes: () => unknown } =>
  typeof value === 'object' && value !== null && 'asBytes' in value && typeof value.asBytes === 'function';

const toBytes = (value: unknown): Uint8Array | null => {
  if (value instanceof Uint8Array) return value;
  if (typeof value === 'string') {
    try {
      return fromHex(value.startsWith('0x') ? value : `0x${value}`);
    } catch {
      return null;
    }
  }
  if (Array.isArray(value)) return new Uint8Array(value);
  if (hasAsBytes(value)) return toBytes(value.asBytes());
  return null;
};
    ...
    async getPeerP256Key(address: string): Promise<Uint8Array | null> {
      ...
      try {
        ...
        if (raw) {
          ...
          // Chain-returned record with a runtime-metadata shape — read the
          // candidate field reflectively instead of casting.
          const record: object = raw;
          const keyCandidate = Reflect.get(record, 'identifier_key') ?? Reflect.get(record, 'identifierKey');
