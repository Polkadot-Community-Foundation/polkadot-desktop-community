...
// A Wolt blurhash maxes out at componentX/Y = 9 → 4 + 2·(81−1) = 164 ASCII
// chars. The wire `thumbnail` is `Option(Bytes)` with no length cap, so a hostile
// peer could stuff arbitrary bytes here; reject anything past a sane ceiling
// before allocating a string for it.
const MAX_BLURHASH_BYTES = 256;

// The wire `thumbnail` is `Option(Bytes)` — the UTF-8 bytes of a blurhash
// string. scale-ts decodes an absent Option to `undefined`/`null`. Recover the
// ASCII blurhash; an empty or oversized payload counts as absent.
const decodeBlurhash = (thumbnail: Uint8Array | null | undefined): string | undefined => {
  if (!thumbnail || thumbnail.length === 0 || thumbnail.length > MAX_BLURHASH_BYTES) return undefined;
  const hash = new TextDecoder().decode(thumbnail);
  return hash.length > 0 ? hash : undefined;
};

const encodeBlurhash = (blurhash: string | undefined): Uint8Array | undefined => {
  if (!blurhash) return undefined;
  return new TextEncoder().encode(blurhash);
};
...
const mapFileMeta = (meta: { tag: string; value: unknown }): FileMeta | null => {
  switch (meta.tag) {
    case 'general': {
    ...
    case 'image': {
      ...
      return {
        ...
        blurhash: decodeBlurhash(meta.value.thumbnail),
      };
    }
    case 'video': {
...
const mapSdkContent = (content: { tag: string; value: unknown }): MessageContent | null => {
  switch (content.tag) {
    case 'text':
...
const mapUiContentToSdk = (content: MessageContent, defaultNodeEndpoint = ''): { tag: string; value: unknown } | null => {
  switch (content.type) {
    case 'text':
            ...
            a.meta.type === 'image'
              ? {
                  ...
                  value: {
                    ...
                    thumbnail: encodeBlurhash(a.meta.blurhash),
                  },
                }
              : a.meta.type === 'video'
                ? {
