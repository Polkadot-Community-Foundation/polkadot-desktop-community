...
import { EncryptedHandshakeResponseV2 } from './schemas';
import { handshakeService } from './service';
import { type HandshakeState } from './types';

const { advance, canSubmitV2Statements, fromInnerResponse, idle, isTerminal, submitted } = handshakeService;
...
describe('fromInnerResponse', () => {
  ...
  it('maps Success to Success state, deriving identityChatPublicKey from the private scalar', () => {
    ...
    if (state.tag === 'Success') {
      ...
      expect(state.identityChatPublicKey).toEqual(x25519.getPublicKey(identityChatPrivateKey));
      expect(state.deviceEncPubKey.length).toBe(32);
    }
  });
