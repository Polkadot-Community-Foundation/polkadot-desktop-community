...
describe('Device', () => {
  it('round-trips a 32-byte accountId and 32-byte public key', () => {
    const d = makeDevice();
...
describe('HandshakeSuccessV2', () => {
  ...
  it('encodes Success with the expected fixed length (32 + 32 + 32 + 32 = 128 bytes)', () => {
    const s = {
      identityAccountId: new Uint8Array(32).fill(0xb2),
      rootAccountId: new Uint8Array(32).fill(0xc3),
      identityChatPrivateKey: new Uint8Array(32).fill(0x05),
    ...
    expect(HandshakeSuccessV2.enc(s).length).toBe(128);
  });
...
describe('HandshakeSuccessV2Legacy', () => {
  ...
  it('encodes to exactly 96 bytes (32 + 32 + 32 — no rootAccountId)', () => {
    const s = {
      identityAccountId: new Uint8Array(32),
      identityChatPrivateKey: new Uint8Array(32),
      deviceEncPubKey: new Uint8Array(32),
    };
    expect(HandshakeSuccessV2Legacy.enc(s).length).toBe(96);
  });
});
...
describe('decodeEncryptedHandshakeResponseV2 (length-dispatched)', () => {
  ...
  it('decodes the v0.2 legacy Success payload (Android feature/location-for-handshake, 96B body), surfaces null rootAccountId', () => {
    ...
    const deviceEncPubKey = new Uint8Array(32).fill(0x04);
    const innerBytes = new Uint8Array(1 + 32 + 32 + 32);
    ...
    expect(innerBytes.length).toBe(97);
