 ...
 * chat X25519 private scalar (32 bytes), and the encryption public key of the
 * authorising PApp device (32 bytes, X25519). Total 128 bytes.
...
/** 32 + 32 + 32 + 32 = 128 bytes — spec v0.2.1 (2026-05-07). */
...
/**
 * 32 + 32 + 32 = 96 bytes — spec v0.2 (2026-05-06). Shipped by Android
 ...
 *
 *   - v0.2 (Android `feature/location-for-handshake`): 96B without
 *     `rootAccountId`.
 *   - v0.2.1: 128B with `rootAccountId`.
 *
...
export const decodeEncryptedHandshakeResponseV2 = (bytes: Uint8Array): DecodedHandshakeResponseV2 => {
  ...
  if (tag === 1) {
    ...
    throw new Error(`EncryptedHandshakeResponseV2: Success body length ${body.length} not in {96, 128}`);
  }
