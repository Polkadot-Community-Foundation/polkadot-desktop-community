      ...
      <div key={attachment.id} className="group relative shrink-0">
        {attachment.previewUrl ? (
          ...
          <div className="flex size-16 flex-col items-center justify-center gap-1 rounded-xl bg-bg-surface-nested">
            ...
            <span className="max-w-14 truncate text-label-small text-fg-tertiary">{formatFileSize(attachment.file.size)}</span>
          </div>
        )}
        <button
          className="absolute -end-1 -top-1 flex size-5 items-center justify-center rounded-full bg-bg-surface-container-inverted opacity-0 transition-opacity group-hover:opacity-100"
