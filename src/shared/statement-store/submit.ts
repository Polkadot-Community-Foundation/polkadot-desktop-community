/**
 * Shared app-policy wrapper over the library's sign-and-submit primitive.
 *
 * A domain-agnostic submit-with-retry helper over an INJECTED
 * `StatementStoreAdapter` — it composes no app entities, so it's a shared
 * primitive that gateways, resources, and use cases may all consume (rather than
 * a use case, which would invert the gateway → use-case dependency).
 *
