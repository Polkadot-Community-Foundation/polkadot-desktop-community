...
export const KeyListingPermissionModal = memo(({ product, requestedIdentifier, onAllow, onDeny, onDismiss }: Props) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="contents" data-testid={TEST_IDS.keyListingPermissionDialog} />
        <ProductDialogHeader product={product} />

        <div className="flex flex-col gap-2 py-3">
          <Dialog.Title>
            <span className="text-2xl leading-8 font-semibold text-fg-primary">
              {t('widget.productContainerBinding.keyListingPermission.title', { requestedIdentifier })}
            </span>
          </Dialog.Title>
          <Dialog.Description>
            <span className="text-base leading-6 font-normal text-fg-primary">
              {t('widget.productContainerBinding.keyListingPermission.subtitle')}
            </span>
          </Dialog.Description>
        </div>
        ...
        <div className="flex w-full items-start gap-2 rounded-lg border border-stroke-primary bg-bg-status-warning/10 p-3">
          ...
          <p className="text-sm leading-5 font-medium text-fg-primary">
            {t('widget.productContainerBinding.keyListingPermission.warning')}
          </p>
        </div>
        <Dialog.Footer>
          <div className="flex w-full min-w-0 gap-2">
            <div className="min-w-0 flex-1">
              <Button type="button" variant="outline" fullWidth onClick={onDeny}>
                {t('widget.productContainerBinding.keyListingPermission.deny')}
              </Button>
            </div>
            <div className="min-w-0 flex-1">
              <Button
                ...
                variant="default"
                fullWidth
                data-testid={TEST_IDS.keyListingPermissionAllow}
                onClick={onAllow}
              >
                {t('widget.productContainerBinding.keyListingPermission.allow')}
              </Button>
            </div>
          </div>
        </Dialog.Footer>
