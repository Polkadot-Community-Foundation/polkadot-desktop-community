...
import { chainService, genesisHash, useAllChainsMap, useApi, useBlockTime } from '@/domains/network';
...
export const SignPayloadWithLegacyAccountModal = memo(
  ({ session, payload, account, productIdentifier, flowId, onCancel, onResult }: Props) => {
    const tag = flowId ? `[Signing][${flowId}][SignPayloadLegacy]` : '[Signing][SignPayloadLegacy]';
    useEffect(() => {
    ...
    const sign = () => {
      ...
      console.info(`${tag} sign() started — calling session.signRawLegacy(Payload)`, {
        address,
      ...
      <SSODialog.Root onDismiss={dismissReviewWithRejectedToast}>
        {step === 'polkadotApp' ? (
          <SSODialog.WaitingForMobile
            ...
            session={session}
            onAbort={() => onCancel(new SigningErr.Rejected())}
          />
        ) : (
          <div className="flex min-h-0 w-full min-w-0 flex-1 flex-col gap-6">
            ...
            {!showDetails ? (
              <div className="pt-2">
                <SSODialog.Title>{requestTitle}</SSODialog.Title>
              </div>
            ) : (
              <button
                type="button"
              ...
              {!showDetails ? (
                <>
                  <div className={signingSummarySectionClassName}>
                    {!canInspectSigning ? (
                      <>
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-sm leading-5 text-fg-secondary">{t('feature.browser.customChainSigningWarning')}</p>
                        </div>
                    ...
                    {canInspectSigning ? (
                      <div className="flex items-start justify-between gap-3 text-base leading-6 text-fg-secondary">
...
SignPayloadWithLegacyAccountModal.displayName = 'SignPayloadWithLegacyAccountModal';
