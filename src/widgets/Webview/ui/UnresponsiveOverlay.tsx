...
export const UnresponsiveOverlay = ({ info, onReload }: Props) => {
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center p-12">
      <div
        className="flex max-w-sm flex-col items-center gap-6 rounded-lg bg-bg-surface-main/85 p-6 shadow-lg backdrop-blur-sm"
        style={{ appRegion: 'no-drag' }}
      >
        <div className="flex flex-col items-center gap-2 self-stretch">
          <div className="pb-2">
            <Loader2 className="h-8 w-8 animate-spin text-fg-secondary" strokeWidth={1.5} />
          </div>
          <h1 className="w-full text-center text-base leading-6 font-medium text-fg-primary">
            {t('widget.webview.unresponsive.title')}
          </h1>
          <p className="w-full text-center text-sm leading-5 font-normal text-fg-secondary">
        ...
        <details className="w-full text-xs text-fg-tertiary" data-testid="unresponsive-overlay-details">
